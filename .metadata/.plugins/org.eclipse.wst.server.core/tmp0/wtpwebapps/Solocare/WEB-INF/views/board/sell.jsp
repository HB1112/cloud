...
<body>
   <body>
    ...
    <div class="content">
        <h2 class="board-title">판매 게시판</h2>
        ...
        <div class="board-card">
            <div class="search-container">
                ...
                <a href="writeboard?category=sell" onclick="return checkLogin();" class="write-button">게시글 작성</a>
            </div>
            ...
            <div class="pagination">
                <c:if test="${currentPage > 1}">
                    <c:choose>
                        <c:when test="${not empty items and not empty text}">
                            <a href="searchboard?category=sell&page=${currentPage - 1}&items=${items}&text=${text}">이전</a>
                        </c:when>
                        <c:otherwise>
                ...
                <c:forEach begin="1" end="${totalPages}" var="i">
                    <c:choose>
                        ...
                        <c:otherwise>
                            <c:choose>
                                <c:when test="${not empty items and not empty text}">
                                    <a href="searchboard?category=sell&page=${i}&items=${items}&text=${text}">${i}</a>
                                </c:when>
                                <c:otherwise>
                ...
                <c:if test="${currentPage < totalPages}">
                    <c:choose>
                        <c:when test="${not empty items and not empty text}">
                            <a href="searchboard?category=sell&page=${currentPage + 1}&items=${items}&text=${text}">다음</a>
                        </c:when>
                        <c:otherwise>
